...
namespace Plugin\TappayJkoPay\Controllers;
...
use Plugin\TappayJkoPay\Services\JkoPayService;

class JkoPayController extends Controller
{
    /**
     * 前端送出後，建立街口支付付款，回傳 payment_url
     */
    public function pay(Request $request): JsonResponse
    {
        try {
            ...
            $result = (new JkoPayService($order))->createPayment($prime);
            ...
            return json_fail($result['msg'] ?? __('TappayJkoPay::common.pay_fail'));

        } catch (\Exception $e) {
            Log::error('TapPay JKO Pay error: ' . $e->getMessage());
    ...
    public function return(Request $request): RedirectResponse
    {
        ...
        if ($number && $recTradeId && $status === '0') {
            try {
                ...
                Log::error('TapPay JKO Pay return error: ' . $e->getMessage());
            }
        }
    ...
    public function notify(Request $request): JsonResponse
    {
        try {
            ...
            Log::info('TapPay JKO Pay notify: ' . json_encode($data));
            ...
            Log::error('TapPay JKO Pay notify error: ' . $e->getMessage());
        }
