...
namespace Plugin\TappayJkoPay\Services;

use Beike\Shop\Services\PaymentService;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;

class JkoPayService extends PaymentService
{
    ...
    /**
     * 建立街口支付付款，回傳 TapPay 的 payment_url
     */
    public function createPayment(string $prime): array
    {
        ...
        $payload = [
            ...
            'result_url'  => [
                'frontend_redirect_url' => route('shop.tappay_jko_pay.return', ['order_number' => $order->number]),
                'backend_notify_url'    => route('shop.tappay_jko_pay.notify'),
                'go_back_url'           => url('/'),
            ],
            'order_number' => $order->number,
        ];

        Log::info('TapPay JKO Pay request: ' . json_encode($payload));

        $response = Http::withHeaders([
            'x-api-key' => $this->partnerKey,
        ])->asJson()->post($this->apiEndpoint, $payload);

        $result = $response->json();
        Log::info('TapPay JKO Pay response: ' . json_encode($result));
