...
namespace Plugin\TappayEasyWallet\Controllers;
...
use Plugin\TappayEasyWallet\Services\EasyWalletService;

class EasyWalletController extends Controller
{
    /**
     * 前端送出後，建立悠遊付付款，回傳 payment_url
     */
    public function pay(Request $request): JsonResponse
    {
        try {
            ...
            $result = (new EasyWalletService($order))->createPayment($prime);
            ...
            return json_fail($result['msg'] ?? __('TappayEasyWallet::common.pay_fail'));

        } catch (\Exception $e) {
            Log::error('TapPay Easy Wallet error: ' . $e->getMessage());
    ...
    public function return(Request $request): RedirectResponse
    {
        ...
        if ($number && $recTradeId && $status === '0') {
            try {
                ...
                Log::error('TapPay Easy Wallet return error: ' . $e->getMessage());
            }
        }
    ...
    public function notify(Request $request): JsonResponse
    {
        try {
            ...
            Log::info('TapPay Easy Wallet notify: ' . json_encode($data));
            ...
            Log::error('TapPay Easy Wallet notify error: ' . $e->getMessage());
        }
