...
未設定此排程會導致「待支付」訂單永遠不會自動取消，庫存也不會釋放。

### 11-1. 後台設定自動取消時數

後台 → **系統** → **基本設定**，找到以下欄位並填入：

| 設定項目 | 建議值 | 說明 |
|---|---|---|
| 訂單自動取消時數 | `24` | 超過 24 小時未付款自動取消 |
| 訂單自動完成時數 | `168` | 出貨後 7 天自動完成訂單 |

### 11-2. 設定 Windows 工作排程器

1. 開始 → 搜尋「工作排程器」→ 開啟
2. 右側點「建立基本工作」
3. 名稱輸入：`BeikeShop 訂單排程`
4. 觸發程序：選「每天」→ 下一步 → 勾選「每隔 1 天重複執行」改為每 **1 分鐘**
5. 動作：選「啟動程式」
6. 填入以下資訊：
   - 程式：`C:\xampp\php\php.exe`
   - 新增引數：`C:\xampp\htdocs\beikeshop\artisan schedule:run`
7. 完成後右鍵該工作 → 執行，確認有正常跑起來
