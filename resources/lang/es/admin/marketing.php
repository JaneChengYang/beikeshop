<?php
/**
 * Lang.php
 *
 * @copyright  2024 beikeshop.com - All Rights Reserved
 * @link       https://beikeshop.com
 * @author     guangda <service@guangda.work>
 * @created    2023-09-09 09:09:09
 * @modified   2024-04-25 10:56:13
 */

return [
    'ali_pay_success'         => '¿Ya ha realizado el pago?',
    'ali_pay_text'            => 'Pago completado, por favor refresque la página',
    'amount'                  => 'Cantidad del pedido',
    'attention_1'             => 'Tenga en cuenta que:',
    'attention_2'             => 'Con cada compra de plugin, sólo se autoriza su uso en el dominio correspondiente a la compra! Está prohibido comprar una vez para usar en más de un dominio. Cualquier violación será considerada legalmente responsable!',
    'attention_3'             => 'Los productos virtuales, que no son problemas del software en sí, no pueden ser devueltos o intercambiados.',
    'attention_show_1'        => 'Nota importante:',
    'attention_show_2'        => 'Con cada compra de plugin, sólo se autoriza su uso en el dominio correspondiente a la compra! Está prohibido comprar una vez para usar en más de un dominio. Cualquier violación será considerada legalmente responsable!',
    'attention_show_3'        => 'El software para los productos especiales, después de pagar la compra no es el software en sí mismo problema no es compatible con el reembolso, por favor, lea atentamente las instrucciones del plugin antes de comprar',
    'bind_bk_token'           => 'Vincular la cuenta US3C y obtener tokens automáticamente',
    'bk_login_hint'           => 'Iniciar sesión con la cuenta beikeshop.com',
    'bk_register_hint'        => 'Registrar una nueva cuenta con beikeshop.com',
    'btn_buy'                 => 'compra',
    'btn_buy_service'         => 'Comprar después de venta',
    'check_token_error'       => 'El token no coincide con el sitio web actualmente visitado. Por favor, vuelva a obtenerlo',
    'create_date'             => 'Hora de orden',
    'days'                    => 'días',
    'download_count'          => 'recuento de descargas',
    'download_description'    => 'Descripción del complemento',
    'download_plugin'         => 'descargar complemento',
    'free_days'               => 'Como regalo',
    'free_days_over'          => 'Mes después de ventas',
    'get_token'               => 'Obtener token',
    'get_token_text'          => 'Ir a la página web oficial de US3C personal center - enlazar el dominio, añadir el dominio actual',
    'last_update'             => 'última actualización',
    'lv1_developer'           => 'Desarrollador en LV1',
    'lv2_developer'           => 'Desarrollador de LV2',
    'lv3_developer'           => 'Desarrollador en LV3',
    'marketing_buy'           => 'Comprar el plugin',
    'marketing_download'      => 'Descargar el plugin',
    'marketing_index'         => 'Mercado home page',
    'marketing_list'          => 'mercado complementario',
    'marketing_show'          => 'detalles del complemento',
    'month'                   => 'Especificaciones para comprar',
    'munths'                  => 'meses',
    'no_choose'               => '¡No seleccionado!',
    'official_developer'      => 'El desarrollador oficial',
    'over_date'               => 'Tiempo de expiración',
    'pay_success_text'        => 'Compra exitosa, haga clic en ok para actualizar la página',
    'pay_success_title'       => 'Pago realizado con éxito!',
    'payment_method'          => 'Formas de pago',
    'plugin_name'             => 'Nombre del plugin',
    'plugin_ticket'           => 'Presentación de órdenes de trabajo',
    'same_domain_error'       => 'El dominio de acceso actual no coincide con el archivo.env APP_URL en la raíz. Modifique',
    'select_pay'              => 'Selecciona el método de pago',
    'service_buy_histories'   => 'Registro de compra',
    'service_date_to'         => 'Vencimiento post-venta',
    'set_token'               => 'Establecer ficha',
    'text_author'             => 'autor del complemento',
    'text_compatibility'      => 'compatibilidad',
    'text_free'               => 'gratis',
    'text_pay'                => 'Cantidad a pagar',
    'text_version'            => 'versión',
    'to_update'               => 'Ir a upgrade',
    'version_compatible_text' => 'Este complemento no es compatible con la versión actual del sistema; actualice a <a href="' . config('beike.official_website') . '/download" target="_blank">última versión </a>',
    'wxpay'                   => 'Wechat barrido código de pago!',
    'yuan'                    => 'yuan',
];
