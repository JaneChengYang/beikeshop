<?php
/**
 * Lang.php
 *
 * @copyright  2024 beikeshop.com - All Rights Reserved
 * @link       https://beikeshop.com
 * @author     guangda <service@guangda.work>
 * @created    2023-09-09 09:09:09
 * @modified   2024-04-25 10:56:55
 */

return [
    'ali_pay_success'         => '지불완료?',
    'ali_pay_text'            => '결제가 완료되었으니 페이지를 새로고침해주세요',
    'amount'                  => '주문 금액',
    'attention_1'             => '주목해 주세요:',
    'attention_2'             => '플러그인을 구매할 때마다, 해당 도메인의 구매에만 사용할 수 있습니다!한 번에 여러 도메인을 구입해서 사용하는 것을 금지하며, 어떠한 위반 행위든지 법적책임을 추궁당할 것입니다!',
    'attention_3'             => '소프트웨어 자체의 문제가 아니면 가상 상품은 일체 교환되지 않는다.',
    'attention_show_1'        => '주의사항:',
    'attention_show_2'        => '플러그인을 구매할 때마다, 해당 도메인의 구매에만 사용할 수 있습니다!한 번에 여러 도메인을 구입해서 사용하는 것을 금지하며, 어떠한 위반 행위든지 법적책임을 추궁당할 것입니다!',
    'attention_show_3'        => '소프트웨어는 특별한 상품입니다. 소프트웨어 이외의 문제는 환불이 되지 않습니다. 구매전에 플러그인의 설명을 자세히 읽어 보십시오',
    'bind_bk_token'           => 'beikeshop 계정을 연동하여 자동으로 토큰을 획득한다',
    'bk_login_hint'           => 'beikeshop.com 계정으로 로그인하세요',
    'bk_register_hint'        => 'beikeshop.com에 새 계정을 등록하세요',
    'btn_buy'                 => '구매',
    'btn_buy_service'         => '구매 애프터서비스',
    'check_token_error'       => '토큰이 현재 방문한 사이트와 일치하지 않습니다. 다시 가져오십시오',
    'create_date'             => '주문 발주 시간',
    'days'                    => '일',
    'download_count'          => '다운로드 횟수',
    'download_description'    => '플러그인 설명',
    'download_plugin'         => '플러그인 다운로드',
    'free_days'               => '선물',
    'free_days_over'          => '일 개월 판매 후',
    'get_token'               => 'Token을 클릭하여 획득합니다',
    'get_token_text'          => 'US3C 홈페이지 개인센터 로그인 - 도메인 이름 바인딩, 현재 도메인 이름 추가',
    'last_update'             => '마지막 업데이트',
    'lv1_developer'           => 'lv1개발자',
    'lv2_developer'           => 'lv2개발자',
    'lv3_developer'           => 'lv3개발자',
    'marketing_buy'           => '플러그인 구매',
    'marketing_download'      => '플러그인 다운로드',
    'marketing_index'         => '마켓 초기화면',
    'marketing_list'          => '플러그인마켓',
    'marketing_show'          => '플러그인 상세',
    'month'                   => '구매 스펙',
    'munths'                  => '개월',
    'no_choose'               => '선택되지 않음!',
    'official_developer'      => '오피니언 개발자',
    'over_date'               => '기한',
    'pay_success_text'        => '플러그인 구매 성공, 페이지 새로 고침 확정 클릭',
    'pay_success_title'       => '결제 성공!',
    'payment_method'          => '지불 방식',
    'plugin_name'             => '플러그인 이름',
    'plugin_ticket'           => '작업 명세서를 제출하다.',
    'same_domain_error'       => '현재 도메인 이름이 루트 디렉터리의.env 파일 app_url과 일치하지 않습니다. 수정해 주십시오',
    'select_pay'              => '결제수단 선택',
    'service_buy_histories'   => '구매 기록',
    'service_date_to'         => '판매후 만기일',
    'set_token'               => 'Token설치',
    'text_author'             => '플러그인 작성자',
    'text_compatibility'      => '호환성',
    'text_free'               => '무료',
    'text_pay'                => '지급금액',
    'text_version'            => '버전',
    'to_update'               => '업그레이드하러 가기',
    'version_compatible_text' => '이 플러그인은 현재 시스템 버전과 호환되지 않습니다. <a href="' . config('beike.official_website') . '/download" target="_blank">최신 버전으로 업그레이드하세요. </a>',
    'wxpay'                   => '위챗 코드 스캔 결제!',
    'yuan'                    => '위안',
];
